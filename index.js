...
function addEventListenerToAllCategoryRows() {
  ...
  categoryRows.forEach((row) => {
    ...
    row.addEventListener('click', () => {
      ...
      messageStorage[row.querySelector('.category-text').textContent].forEach((messageObj) => {
...
function handleNewMessage() {
  ...
  const messageContainer = document.querySelector('.messages-container');
  const selectedRow = document.querySelector('.category-row.selected .category-text');
  const newMessageDiv = document.createElement('div');
  ...
  fetch(`https://v2.jokeapi.dev/joke/${selectedRow.textContent}?blacklistFlags=nsfw,religious,political,racist,sexist,explicit&type=single&amount=1`)
    .then((res) => res.json())
    .then((data) => {
